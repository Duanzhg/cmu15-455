 ...
 public:
  ...
  void PerformChecks(ExecutorContext *exec_ctx) {
    for (const auto &[left_executor, right_executor] : exec_ctx->GetNLJCheckExecutorSet()) {
     ...
     // std::cout << casted_left_executor->GetInitCount()+1 << " " << casted_left_executor->GetNextCount() << std::endl;
      BUSTUB_ASSERT( true || casted_right_executor->GetInitCount() + 1 >= casted_left_executor->GetNextCount(),
